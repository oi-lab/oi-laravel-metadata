...
```bash
php artisan oi:skills oilab-laravel-metadata --project
# or, for the global scope (~/.claude)
php artisan oi:skills oilab-laravel-metadata --global
```

Installing this skill:

- writes `SKILL.md` to `.claude/skills/oilab-laravel-metadata/` (Claude Code) and
  `.junie/skills/oilab-laravel-metadata/` (JetBrains Junie);
- adds (or refreshes) an `=== oi-lab/oi-laravel-metadata rules ===` section in your project's `CLAUDE.md`.

> A package-local command `php artisan oi-metadata:install-ai-skill` is still available for projects that don't
> use `oi-lab/oi-laravel-development`, but it is **deprecated** in favor of `oi:skills`.
