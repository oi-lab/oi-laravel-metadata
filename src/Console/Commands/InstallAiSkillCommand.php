...
{
    protected $signature = 'oi-metadata:install-ai-skill';

    protected $description = '[Deprecated] Use `oi:skills` instead. Install AI assistant skill files for oi-laravel-metadata';

    private const SKILL_NAME = 'oilab-laravel-metadata';

    private const SECTION = 'oi-lab/oi-laravel-metadata rules';
    ...
    private function addSkillToClaudeMd(): void
    {
        ...
        if (! file_exists($claudeMdPath)) {
            ...
            $this->info('Created CLAUDE.md with oi-laravel-metadata rules.');
        ...
        if (! str_contains($content, $sectionHeader)) {
            ...
            $this->info('Added oi-laravel-metadata rules section to CLAUDE.md.');
        ...
        $this->info('Updated oi-laravel-metadata rules section in CLAUDE.md.');
    }
}
